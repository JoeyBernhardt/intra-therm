...
biotime <- read.csv("data-processed/intratherm-biotime_nikki.csv")%>%
	arrange(STUDY_ID) ## studies have unique STUDY_ID
biotime_with_absence <- read.csv("data-processed/intratherm-biotime-with-absence_nikki.csv") %>%
	arrange(STUDY_ID) ## studies have unique STUDY_ID
gpdd <- read_csv("data-processed/intratherm-gpdd_nikki.csv") %>%
